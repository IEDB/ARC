...
class SeqClassifier:
    ...
    def run_hmmscan(self, seq_record, hmm_out):
        ...
        with tempfile.NamedTemporaryFile(mode="w") as temp_out:
            ...
            args = ['hmmscan', '-o', hmm_out.name, os.path.join(
                self.package_directory, "data/HMMs/ALL_AND_C.hmm"), temp_out.name]
            cmd = (' ').join(args)
    ...
    def assign_Gdomain(self, seq, seq_id=None):
        ...
        """
        gd = mhc_G_domain(chain1_seq=seq, ch1_id=re.sub(
            r'[^\w|\.]', '', seq_id))
        res = gd.get_g_domain()
        if res:
            return res
    ...
    def get_MRO_Gdomains(self, mro_TSVfile):
        ...
        for i in list(range(mro.shape[0])):
            ...
            mro_out.loc[cnt, 'calc_mhc_class'], mro_out.loc[cnt, 'ch_g_dom'] = self.assign_Gdomain(
                mro.loc[i, 'Sequence'], mro.loc[i, 'Accession'])
            cnt += 1
    ...
    def get_MRO_allele(self, mro_df, seq, seq_id=None):
        ...
        if pdb_g_dom:
            mro_allele = str(list(mro_df[mro_df.fillna({'ch_g_dom': ''}).apply(lambda r: r['ch_g_dom'] != '' and (
    ...
    def is_ignar(self, sequence):
      ...
      with tempfile.NamedTemporaryFile(mode="w") as temp_in:
        ...
        with open('/tmp/ignar_out.xml', 'r') as outfile:

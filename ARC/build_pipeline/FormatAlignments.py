    ...
    with open( filename, "w") as outfile:
        ...
        for c_name in all_C_alignments:
            for species, chain_type in all_C_alignments[c_name] :
                write_stockholm( all_C_alignments[c_name][(species, chain_type) ], "%s_%s%s"%(translations[species], chain_type,c_name), outfile)
    return filename      
...
def main():
    ...
    #combined_sequences = make_putatitve_alignments( valignments, jalignments )    
